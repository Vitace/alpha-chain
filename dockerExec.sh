...
echo "Running alphachain inside docker with the following arguments:
  HOST:  $HOST
  PORT:  $PORT
  LOG_LEVEL:  $LOG_LEVEL
  PEERS: $PEERS
...
docker run -t --rm --name alphachain_$NAME $HOST_ARG $NAME_ARG $PORT_ARG $PEERS_ARG $LOG_LEVEL_ARG -v /$(pwd):/alphachain -p $PORT:$PORT alphachain